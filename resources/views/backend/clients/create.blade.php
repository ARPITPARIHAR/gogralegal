...
@section('meta_title',__('Add Client'))

@section('page_name',__('Add Client'))

@section('page_description',__('Add Client'))
@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('admin.dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __('Add Client') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
            ...
            <div class="card-block">
                <form action="{{ route('admin.clients.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="url" name="url" id="url" value="{{ old('url')}}" placeholder="{{ __('Enter Url') }}" class="form-control @error('url') form-control-danger @enderror">
                            @error('url')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
