...
@section('meta_title',__('Add Case'))

@section('page_name',__('Add Case'))

@section('page_description',__('Add Case'))
@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('admin.dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __('Add Case') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
            ...
            <div class="card-block">
                <form action="{{ route('admin.billings.store') }}" method="POST">
                        ...
                        <div class="col-sm-10">
                            <select name="customer" class="form-control @error('customer') form-control-danger @enderror">
                                ...
                                @foreach (App\Models\User::whereIn('user_type',['customer'])->orderBy('name')->get() as $customer)
                                    <option value="{{ $customer->id }}" @selected($customer->id==old('customer'))>{{ $customer->name }}</option>
                                @endforeach
                            </select>
                            <span class="messages">
                                @error('customer')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" placeholder="{{ __('Enter Amount') }}" class="form-control @error('amount') form-control-danger @enderror">
                           <span class="messages">
                                @error('amount')
                                    <p class="text-danger error">{{ $message }}</p>
                                @else
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date') ?? date('Y-m-d') }}" placeholder="{{ __('Choose Payment Date') }}" class="form-control @error('payment_date') form-control-danger @enderror">
                           <span class="messages">
                                @error('payment_date')
                                    <p class="text-danger error">{{ $message }}</p>
                                @else
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select name="payment_mode" class="form-control @error('payment_mode') form-control-danger @enderror">
                                ...
                                <option value="cash" @selected('cash'==old('payment_mode'))>{{ __('Cash') }}</option>
                                <option value="online" @selected('online'==old('payment_mode'))>{{ __('Online') }}</option>
                                <option value="cheque" @selected('cheque'==old('payment_mode'))>{{ __('Cheque') }}</option>
                                <option value="other" @selected('other'==old('payment_mode'))>{{ __('Other') }}</option>
                            </select>
                            <span class="messages">
                                @error('payment_mode')
                                    <p class="text-danger error">{{ $message }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="description" id="description" placeholder="{{ __('Enter Description') }}" class="form-control @error('description') form-control-danger @enderror">{{ old('description')}}</textarea>
                            @error('description')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
