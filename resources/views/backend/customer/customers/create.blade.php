...
@section('meta_title',__('Add Customer'))

@section('page_name',__('Add Customer'))

@section('page_description',__('Add Customer'))
@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('admin.dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __('Add Customer') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
            ...
            <div class="card-block">
                <form action="{{ route('admin.customers.store') }}" method="POST">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="{{ __('Enter Email') }}" class="form-control @error('email') form-control-danger @enderror">
                            @error('email')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="{{ __('Enter Phone') }}" class="form-control @error('phone') form-control-danger @enderror">
                            @error('phone')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="address" id="address" placeholder="{{ __('Enter Address') }}" class="form-control @error('address') form-control-danger @enderror">{{ old('address')}}</textarea>
                            @error('address')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
