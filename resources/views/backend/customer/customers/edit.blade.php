...
@section('meta_title',__('Edit Customer'))

@section('page_name',__('Edit Customer'))

@section('page_description',__('Edit Customer'))
@section('name')
    <li class="breadcrumb-item">
        <a href="{{ route('admin.dashboard') }}"> <i class="feather icon-home"></i> </a>
    </li>
    <li class="breadcrumb-item"><a href="#!">{{ __('Edit Customer') }}</a>
    </li>
@endsection
@section('content')
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="card">
            ...
            <div class="card-block">
                <form action="{{ route('admin.customers.update',encrypt($user->id)) }}" method="POST">
                    @csrf
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">{{ __('Name') }}</label>
                        <div class="col-sm-10">
                            <input type="text" name="name" id="name" value="{{ old('name') ?? $user->name }}" placeholder="{{ __('Enter Name') }}" class="form-control @error('name') form-control-danger @enderror">
                           <span class="messages">
                                @error('name')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="email" name="email" id="email" value="{{ old('email') ?? $user->email }}" placeholder="{{ __('Enter Email') }}" class="form-control @error('email') form-control-danger @enderror">
                            @error('email')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="phone" id="phone" value="{{ old('phone') ?? $user->phone }}" placeholder="{{ __('Enter Phone') }}" class="form-control @error('phone') form-control-danger @enderror">
                            @error('phone')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="address" id="address" placeholder="{{ __('Enter Address') }}" class="form-control @error('address') form-control-danger @enderror">{{ old('address') ?? $user->address}}</textarea>
                            @error('address')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
                                <p class="text-muted">{{ __('') }}</p>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="password" name="password" id="password" value="{{ old('password') }}" placeholder="{{ __('Enter Password') }}" class="form-control @error('password') form-control-danger @enderror">
                            @error('password')
                                <p class="text-danger error">{{ $message }}</p>
                            @else
